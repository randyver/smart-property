...
export default function AnalyticsPage() {
  ...
  const getClimateDataForChart = () => {
    ...
    return climateByDistrict
      .filter(district => district[activeClimateMetric] !== null && !isNaN(district[activeClimateMetric]))
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Property Market Analytics</h2>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-white p-4 rounded-lg shadow-md">
                  ...
                  <p className="text-2xl font-bold text-gray-800">
                    {formatter.formatPercentage(summaryData?.climate_safe_percentage || 0, 1)}
                  </p>
                </div>
                <div className="bg-white p-4 rounded-lg shadow-md">
                  <h3 className="text-sm font-medium text-gray-500">Avg Climate Score</h3>
                  <div className="flex items-center">
                    <div className={`w-3 h-3 rounded-full mr-2 ${formatter.getClimateScoreColor(summaryData?.avg_climate_scores?.overall)}`}></div>
                    <p className="text-2xl font-bold text-gray-800">
                      {Math.round(summaryData?.avg_climate_scores?.overall || 0)}
                    </p>
                  </div>
                </div>
              </div>
              
              <Tabs defaultValue="price" className="mb-6" onValueChange={setActiveTab}>
                ...
                <TabsContent value="climate" className="mt-0">
                  ...
                  <div className="bg-white p-4 rounded-lg shadow-md mb-6">
                    <h3 className="font-medium mb-2 text-black">Climate Metric</h3>
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(climateMetricLabels).map(([key, label]) => (
                        <button
                          key={key}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    {climateImpactData.length > 0 && activeClimateMetric === 'overall_score' && (
                      <AnalyticsChart
                        type="bar"
                    ...
                    {climateImpactData.length > 0 && activeClimateMetric === 'lst_score' && (
                      <AnalyticsChart
                        type="bar"
                    ...
                    {climateImpactData.length > 0 && activeClimateMetric === 'ndvi_score' && (
                      <AnalyticsChart
                        type="bar"
                    ...
                    {climateImpactData.length > 0 && activeClimateMetric === 'utfvi_score' && (
                      <AnalyticsChart
                        type="bar"
                    ...
                    {climateImpactData.length > 0 && activeClimateMetric === 'uhi_score' && (
                      <AnalyticsChart
                        type="bar"
                  ...
                  <div className="bg-white p-6 rounded-lg shadow-md mb-6">
                    <h3 className="text-lg font-bold mb-4 text-black">Climate Impact Analysis</h3>
                    <p className="text-gray-700 mb-4">
                      Our analysis shows how climate factors affect property values across different districts.
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 text-black">
                      <div className="border p-4 rounded-md">
                        ...
                        <ul className="list-disc pl-5 space-y-1 text-gray-700">
                          <li>Properties with high vegetation index (NDVI) show up to 15% higher valuation</li>
                          <li>Areas with lower surface temperatures typically have higher property values</li>
                          <li>Urban heat island effect correlates with reduced property prices</li>
                          <li>Climate-safe properties maintain value better over time</li>
                        </ul>
                      </div>
                      ...
                      <div className="border p-4 rounded-md">
                        ...
                        <ul className="list-disc pl-5 space-y-1 text-gray-700">
                          <li>Prioritize districts with higher overall climate scores</li>
                          <li>Consider properties with good vegetation coverage</li>
                          <li>Look for areas with effective urban cooling strategies</li>
                          <li>Invest in climate-forward property improvements to increase value</li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {summaryData && summaryData.avg_climate_scores && (
                      <div className="bg-white p-4 rounded-lg shadow-md text-black">
                        <h3 className="font-bold text-lg mb-4">Average Climate Scores</h3>
                        <div className="space-y-4">
                          <div>
                            <div className="flex justify-between mb-1">
                              <span className="text-sm font-medium">Land Surface Temperature</span>
                              <span className="text-sm font-medium">{Math.round(summaryData.avg_climate_scores.lst || 0)}</span>
                            </div>
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-red-500" 
                                style={{ width: `${summaryData.avg_climate_scores.lst}%` }}
                              ></div>
                            </div>
                          </div>
                          ...
                          <div>
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="text-sm font-medium">{Math.round(summaryData.avg_climate_scores.ndvi || 0)}</span>
                            </div>
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-green-500" 
                                style={{ width: `${summaryData.avg_climate_scores.ndvi}%` }}
                              ></div>
                            </div>
                          </div>
                          ...
                          <div>
                            <div className="flex justify-between mb-1">
                              <span className="text-sm font-medium">Urban Thermal Index (UTFVI)</span>
                              <span className="text-sm font-medium">{Math.round(summaryData.avg_climate_scores.utfvi || 0)}</span>
                            </div>
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-yellow-500" 
                                style={{ width: `${summaryData.avg_climate_scores.utfvi}%` }}
                              ></div>
                            </div>
                          </div>
                          ...
                          <div>
                            <div className="flex justify-between mb-1">
                              <span className="text-sm font-medium">Urban Heat Island (UHI)</span>
                              <span className="text-sm font-medium">{Math.round(summaryData.avg_climate_scores.uhi || 0)}</span>
                            </div>
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-purple-500" 
                                style={{ width: `${summaryData.avg_climate_scores.uhi}%` }}
                              ></div>
                            </div>
                          </div>
                          ...
                          <div>
                            <div className="flex justify-between mb-1">
                              <span className="text-sm font-medium font-bold">Overall Climate Score</span>
                              <span className="text-sm font-medium font-bold">{Math.round(summaryData.avg_climate_scores.overall || 0)}</span>
                            </div>
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-blue-600" 
                                style={{ width: `${summaryData.avg_climate_scores.overall}%` }}
                              ></div>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-md text-black">
                      <h3 className="font-bold text-lg mb-4">Property Market Overview</h3>
                      <div className="space-y-4">
                        <p className="text-gray-700">
                          The Bandung property market shows significant variation in prices across different
                          districts, with climate factors playing an important role in property valuation.
                        </p>
                          ...
                          <ul className="space-y-2 text-gray-700">
                            <li className="flex justify-between">
                              ...
                              <span className="font-medium">
                                {distributionData && distributionData.price_distribution && 
                                  `${formatter.formatCurrency(distributionData.price_distribution[0].min)} - ${formatter.formatCurrency(distributionData.price_distribution[distributionData.price_distribution.length-1].max || 0)}`}
                              </span>
                            </li>
                            <li className="flex justify-between">
                              <span>Average Price:</span>
                              <span className="font-medium">{formatter.formatCurrency(summaryData?.average_price || 0)}</span>
                            </li>
                            <li className="flex justify-between">
                              <span>Most Common Property Type:</span>
                              <span className="font-medium">
                                {getPropertyTypeData().length > 0 && 
                                  getPropertyTypeData().sort((a: { count: number; }, b: { count: number; }) => b.count - a.count)[0].type}
                              </span>
                            </li>
                          </ul>
                        </div>
                        ...
                        <p className="text-gray-700">
                          Properties with higher climate safety scores tend to command premium prices,
                          suggesting that environmental factors are increasingly important in the Bandung
                          real estate market.
                        </p>
                      </div>
                    </div>
